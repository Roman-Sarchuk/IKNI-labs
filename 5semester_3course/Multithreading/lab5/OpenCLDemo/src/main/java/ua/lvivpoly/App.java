...
public class App {
    ...
    static int n;
    static float[][] A, A1, A2, B2, C2, Y3;
    static float[] b, b1, c1, y1, y2, x;

    static float[][] tempM1, tempM2;
    static float[] tempV1, tempV2, tempV3, tempV4;
    static float tempC;

    public static void main(String[] args) {
        // === OCL registry ===
        AlgebraUtilsOCL algebraUtils = new AlgebraUtilsOCL();
        algebraUtils.initCL();
        ...
        // calc b & C2;
        System.out.println("");
        threadPool.start(0, () -> algebraUtils.calcVectorb(b));
        threadPool.start(1, () -> algebraUtils.calcMatrixC2(C2));
        ...
        while (true) {
            ...
            if (scanner.hasNextInt()) {
                ...
                if (option == 1) {
                    System.out.println("");
                    // fill randomly
                    threadPool.start(2, () -> fillVectorRandomly("b1", b1, 1.0f, 10.0f));
                    threadPool.start(3, () -> fillVectorRandomly("c1", c1, 1.0f, 10.0f));
                    threadPool.start(4, () -> fillMatrixRandomly("A", A, 1.0f, 10.0f));
                    threadPool.start(5, () -> fillMatrixRandomly("A1", A1, 1.0f, 10.0f));
                    threadPool.start(6, () -> fillMatrixRandomly("A2", A2, 1.0f, 10.0f));
                    threadPool.start(7, () -> fillMatrixRandomly("B2", B2, 1.0f, 10.0f));
                    threadPool.cycleJoin(8);
                    break;
                }
                else if (option == 2) {
                    System.out.println("");
        ...
        // end
        algebraUtils.releaseCL();
    }

    // Matrix methods
    public static void fillMatrixRandomly(String name, float[][] data, float minRand, float maxRand) {
        for (int i = 0; i < n; i++) {
            for (int j = 0; j < n; j++) {
                data[i][j] = ThreadLocalRandom.current().nextFloat(minRand, maxRand);
            }
        }
        System.out.println(name + " is filled randomly");
    }

    public static void inputMatrix(Scanner scanner, String name, float[][] data) {
    ...
    public static void printMatrix(String name, float[][] data) {
    ...
    // Vector methods
    public static void fillVectorRandomly(String name, float[] data, float minRand, float maxRand) {
        for (int i = 0; i < n; i++) {
            data[i] = ThreadLocalRandom.current().nextFloat(minRand, maxRand);
        }
        System.out.println(name + " is filled randomly");
    }

    public static void inputVector(Scanner scanner, String name, float[] data) {
    ...
    public static void printVector(String name, float[] data) {
        synchronized (System.out) {
            System.out.println("Vector " + name + ":");
            System.out.print("|\t");
            for (int i = 0; i < n; i++) {
